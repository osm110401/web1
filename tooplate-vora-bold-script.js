...
document.querySelectorAll('a, button, .work__item, .service-row').forEach(el => {
  el.addEventListener('mouseenter', () => {
    ...
    ring.style.borderColor = "blue";
    dot.style.background = "blue"; // 점 색상도 파란색으로 변경
    dot.style.opacity = '0.3';
  });
  el.addEventListener('mouseleave', () => {
    ring.style.transform = 'translate(-50%,-50%) scale(1)';
    ring.style.borderColor = "blue"; // 혹은 기본 색상 (예: "black")
    dot.style.background = "blue"; // 점 색상 유지 또는 기본색 변경
    dot.style.opacity = '1';
  });
});
